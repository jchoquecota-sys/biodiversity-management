...
echo "\n=== SISTEMA COMPLETO DE DESCARGA DE IMÁGENES ===\n\n";
...
function downloadImage($url, $filename, $speciesName) {
    ...
    if ($imageData !== false) {
        ...
        if ($imageInfo !== false) {
            echo "✓ Imagen válida: {$imageInfo[0]}x{$imageInfo[1]} pixels\n";
...
echo "PASO 1: Descargando imágenes de especies específicas...\n\n";

$downloadedCount = 0;
$updatedCount = 0;

foreach ($imageSources as $scientificName => $urls) {
...
echo "Especies procesadas: " . count($imageSources) . "\n";
...
echo "¡Sistema de descarga de imágenes completado!\n";
echo "Imágenes guardadas en: $imageDir\n";
echo "Fuentes: Wikimedia Commons, iNaturalist, GBIF\n";
