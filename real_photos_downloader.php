...
echo "\n=== DESCARGADOR DE FOTOGRAFÍAS REALES DE ESPECIES ===\n\n";

// Función para descargar imagen real con cURL
function downloadRealImage($url, $filename, $speciesName) {
...
$realPhotoUrls = [
    // MAMÍFEROS PERUANOS - Fotografías reales verificadas
    'Vicugna vicugna' => [
    ...
    // AVES PERUANAS - Fotografías reales verificadas
    'Vultur gryphus' => [
    ...
    // ESPECIES MARINAS PERUANAS - Fotografías reales verificadas
    ...
    // REPTILES PERUANOS - Fotografías reales verificadas
    'Liolaemus tacnae' => [
    ...
    // ANFIBIOS PERUANOS - Fotografías reales verificadas
...
echo "PASO 1: Descargando fotografías reales de especies peruanas...\n\n";

$downloadedCount = 0;
$updatedCount = 0;

foreach ($realPhotoUrls as $scientificName => $urls) {
    ...
    if ($specie) {
        ...
        foreach ($urls as $index => $url) {
            ...
            if (downloadRealImage($url, $filepath, "Fotografía $imageNumber")) {
...
echo "Especies procesadas: " . count($realPhotoUrls) . "\n";
...
$stmt = $pdo->query("SELECT 
    ...
    COUNT(CASE WHEN image_path LIKE 'images/species/%' THEN 1 END) as with_synthetic_images
...
echo "Con fotografías reales: {$stats['with_real_photos']}\n";
echo "Con imágenes sintéticas: {$stats['with_synthetic_images']}\n";
