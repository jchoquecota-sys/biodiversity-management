        ...
        <div class="card-body">
            <form action="{{ route('admin.publications.update', $publication->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="author" id="author" class="form-control @error('author') is-invalid @enderror" value="{{ old('author', $publication->author) }}" required>
                            @error('author')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="number" name="publication_year" id="publication_year" class="form-control @error('publication_year') is-invalid @enderror" value="{{ old('publication_year', $publication->publication_year) }}" min="1800" max="{{ date('Y') }}" required>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" name="journal" id="journal" class="form-control @error('journal') is-invalid @enderror" value="{{ old('journal', $publication->journal) }}">
                            @error('journal')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" name="doi" id="doi" class="form-control @error('doi') is-invalid @enderror" value="{{ old('doi', $publication->doi) }}" placeholder="10.xxxx/xxxxx">
                            @error('doi')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="abstract" id="abstract" class="form-control @error('abstract') is-invalid @enderror" rows="5">{{ old('abstract', $publication->abstract) }}</textarea>
                    @error('abstract')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="biodiversity_categories[]" id="biodiversity_categories" class="form-control select2" multiple>
                        @foreach($biodiversityCategories as $category)
                            <option value="{{ $category->id }}" {{ in_array($category->id, old('biodiversity_categories', $publication->biodiversityCategories->pluck('id')->toArray())) ? 'selected' : '' }}>
    ...
    <script>
        $(document).ready(function() {
            ...
            function generateCategoryFields(categoryIds) {
                let html = '';
                const existingCategories = {!! json_encode($publication->biodiversityCategories->keyBy('id')->toArray()) !!};
                
                categoryIds.forEach((id, index) => {
                    const categoryName = $('#biodiversity_categories option[value="' + id + '"]').text();
                    const category = existingCategories[id] || {};
                        ...
                        <div class="card mb-3">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="relevant_excerpts[${index}]" class="form-control" rows="3">${pivot.relevant_excerpt || ''}</textarea>
                                </div>
                                <div class="form-group">
                                    <label>Referencia de Página</label>
                                    <input type="text" name="page_references[${index}]" class="form-control" placeholder="ej. p. 45-47" value="${pivot.page_reference || ''}">
                                </div>
                            </div>
                        </div>
                    `;
