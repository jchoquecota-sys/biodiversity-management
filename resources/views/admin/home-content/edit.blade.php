...
@section('title', 'Editar Contenido del Home')

@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <h1>Editar Contenido del Home</h1>
    ...
    <div class="row">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <select name="section" id="section" class="form-control @error('section') is-invalid @enderror" required>
                                ...
                                @foreach($sections as $key => $title)
                                    <option value="{{ $key }}" {{ old('section', $homeContent->section) == $key ? 'selected' : '' }}>
                                        {{ $title }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="key" id="key" class="form-control @error('key') is-invalid @enderror" 
                                   value="{{ old('key', $homeContent->key) }}" placeholder="Ej: title, subtitle, description" required>
                            @error('key')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <select name="type" id="type" class="form-control @error('type') is-invalid @enderror" required>
                                ...
                                <option value="text" {{ old('type', $homeContent->type) == 'text' ? 'selected' : '' }}>Texto</option>
                                <option value="image" {{ old('type', $homeContent->type) == 'image' ? 'selected' : '' }}>Imagen</option>
                                <option value="url" {{ old('type', $homeContent->type) == 'url' ? 'selected' : '' }}>URL</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group" id="value-group">
                            ...
                            <textarea name="value" id="value" class="form-control @error('value') is-invalid @enderror" 
                                      rows="3" placeholder="Ingrese el contenido..." required>{{ old('value', $homeContent->value) }}</textarea>
                            @error('value')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group" id="image-group" style="display: none;">
                            ...
                            @if($homeContent->type === 'image' && $homeContent->getFirstMediaUrl('images'))
                                <div class="mb-2">
                                    <img src="{{ $homeContent->getFirstMediaUrl('images') }}" alt="Imagen actual" class="img-thumbnail" style="max-width: 200px;">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="sort_order" id="sort_order" class="form-control @error('sort_order') is-invalid @enderror" 
                                   value="{{ old('sort_order', $homeContent->sort_order) }}" min="0">
                            @error('sort_order')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            <div class="custom-control custom-switch">
                                <input type="checkbox" class="custom-control-input" id="is_active" name="is_active" value="1" {{ old('is_active', $homeContent->is_active) ? 'checked' : '' }}>
        ...
        <div class="col-md-4">
            <div class="card">
                ...
                <div class="card-body">
                    <p><strong>ID:</strong> {{ $homeContent->id }}</p>
                    <p><strong>Creado:</strong> {{ $homeContent->created_at->format('d/m/Y H:i') }}</p>
                    <p><strong>Actualizado:</strong> {{ $homeContent->updated_at->format('d/m/Y H:i') }}</p>
                    <p><strong>Estado:</strong> 
                        <span class="badge badge-{{ $homeContent->is_active ? 'success' : 'secondary' }}">
                            {{ $homeContent->is_active ? 'Activo' : 'Inactivo' }}
                        </span>
                    </p>
                </div>
            </div>
